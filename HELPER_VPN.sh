...
test_banner() {
  local port="$1" out
  # Method 1: SSH client capture (works when OpenSSH relays banner text)
  ...
  if [[ "$out" == *"HELPER"*      ]] || [[ "$out" == *"I AM BACK"*  ]] || \
     [[ "$out" == *"center"*      ]] || [[ "$out" == *"font color"* ]]; then
    return 0
  fi
  # Method 2 (fallback): Some OpenSSH client builds suppress the banner
  # in BatchMode before displaying it. Verify the banner IS configured by
  # checking: service active + port listening + -b flag in running process.
  systemctl is-active --quiet dropbear         || return 1
  ss -tlnp 2>/dev/null | grep -q ":${port} "  || return 1
  pgrep -a dropbear 2>/dev/null | grep -q -- "-b" && [[ -s "$BANNER_FILE" ]]
}
...
# FIX: Generate any missing Dropbear host keys BEFORE starting the service.
# "Failed loading dropbear_dss_host_key" crashes the first child connection
# and can cause the main process to exit via signal. Pre-generating all key
# types (DSS silently skipped on builds that dropped DSS support) prevents
# this. Keys that already exist are left untouched.
mkdir -p /etc/dropbear
for _kt in rsa ecdsa ed25519; do
  _kf="/etc/dropbear/dropbear_${_kt}_host_key"
  [[ -f "$_kf" ]] || dropbearkey -t "$_kt" -f "$_kf" >/dev/null 2>&1 || true
done
...
resolved=$(getent hosts "$newdom" 2>/dev/null | awk '{print $1}' | head -1)
if [[ -z "$resolved" ]]; then
...
# server_name AND the ssl_certificate paths both embed the domain as
# plain text in this file — a literal find/replace updates all of them
# in one pass instead of needing separate regexes for each directive.
cp /etc/nginx/conf.d/autoscript.conf /tmp/autoscript.conf.bak
sed -i "s/${olddom}/${newdom}/g" /etc/nginx/conf.d/autoscript.conf
...
bot
read -rp "$(echo -e " ${GOLD}${BOLD}Choose [0-26]: ${NC}")" opt
case $opt in
